...
const SelectTags = class {
  ...
  render() {
    ...
    return (index.h("ion-item", { ref: (el) => (this.itemEl = el), lines: this.lines }, this.label && (index.h("ion-label", { position: this.labelPosition }, this.label)), index.h("ion-select", { title: this.placeholder || this.name, disabled: this.disabled, multiple: this.multiple, name: this.name, value: this.value, ref: (el) => (this.choicesEl = el) }, index.h("slot", null), !this.multiple && this.placeholder ? (index.h(OptionEl, { placeholder: true }, this.placeholder)) : null, this.options.map((option) => (index.h("ion-select-option", { value: option.value }, option.label)))), this.allowAdding && (index.h("ion-button", { slot: "end", size: "large", fill: "clear", shape: "round", onClick: () => this.addTag() }, index.h("ion-icon", { slot: "icon-only", name: this.addIcon })))));
  }
