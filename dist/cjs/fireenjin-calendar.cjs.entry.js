...
const Calendar = class {
  constructor(hostRef) {
    ...
    this.fireenjinDateSelected = index.createEvent(this, "fireenjinDateSelected", 7);
    this.fireenjinCalendarNavigate = index.createEvent(this, "fireenjinCalendarNavigate", 7);
    /**
  ...
  renderLegend() {
    ...
    for (let day = 1; day <= 7; day++) {
      date.setDate(day);
      weekDays.push(index.h("div", { class: "head-cell" }, formater.format(date)));
    }
    return weekDays;
  }
  render() {
    return (index.h("div", { class: "calendar-wrapper" }, index.h("ion-grid", { class: "calendar-controls" }, index.h("ion-row", null, index.h("ion-col", { size: "1", onClick: event => this.navigate(event, true) }, index.h("ion-icon", { name: "arrow-back" })), index.h("ion-col", { class: "calendar-title", size: "10", onClick: event => this.switchView(event, this.currentView === "calendar" ? "months" : "years") }, this.getTitle()), index.h("ion-col", { size: "1", onClick: event => this.navigate(event) }, index.h("ion-icon", { name: "arrow-forward" })))), index.h("div", { class: {
        "calendar-grid": true,
    ...
    ]), index.h("div", { class: { "month-grid": true, viewing: this.currentView === "months" } }, index.h("ion-grid", null, index.h("ion-row", null, this.getMonths().map((month, index$1) => (index.h("ion-col", { size: "4", onClick: event => this.selectMonth(event, index$1) }, index.h("div", { class: { cell: true, selected: index$1 === this.month } }, month))))))), index.h("div", { class: { "year-grid": true, viewing: this.currentView === "years" } }, index.h("ion-grid", null, index.h("ion-row", null, this.getYears().map(year => (index.h("ion-col", { size: "4", onClick: event => this.selectYear(event, year) }, index.h("div", { class: { cell: true, selected: year === this.year } }, year)))))))));
  }
};
Calendar.style = calendarCss;

exports.fireenjin_calendar = Calendar;
